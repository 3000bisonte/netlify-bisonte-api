...
export default function GoogleSignIn() {
  ...
  useEffect(() => {
    ...
    const isWebViewDetected = /wv|WebView/i.test(userAgent) ||
    ...
    console.log('WebView detectado:', isWebViewDetected);
  ...
  const handleSignIn = async () => {
    ...
    try {
      console.log('Iniciando login, WebView:', isWebView);
      
      if (isWebView) {
        // Para WebView: usar window.location directa al endpoint de NextAuth
        const baseUrl = window.location.origin;
        const signInUrl = `${baseUrl}/api/auth/signin/google?callbackUrl=${encodeURIComponent(baseUrl + '/home')}`;
        
        console.log('Redirigiendo a:', signInUrl);
        ...
        // Redirección directa para WebView
        window.location.href = signInUrl;
        
        // Limpiar timeout si la redirección fue exitosa
        clearTimeout(timeout);
        
      } else {
        // Para navegadores normales
        console.log('Usando NextAuth signIn para navegador');
